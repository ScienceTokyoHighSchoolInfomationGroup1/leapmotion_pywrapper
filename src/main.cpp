...
PYBIND11_MODULE(leap, m) {
    ...
    py::class_<_LEAP_VECTOR>(m, "Vector")
    ...
    py::class_<_LEAP_QUATERNION>(m, "Quaternion")
    ...
    py::class_<_LEAP_TRACKING_EVENT>(m, "TrackingEvent")
    ...
    // auto dt = m.def_submodule("datatypes", "Leap Motion data types");

    // init_datatypes(dt); // Initialize the datatypes submodule
}